...
[#if ctx.featureManager.limitedTimeQuarantineEnabled]
    [@decorators.displayHtmlHeader requireResourcesForContext=["atl.general", "bamboo.result.quarantine.limited"]/]
[#else]
    [@decorators.displayHtmlHeader requireResourcesForContext=["atl.general", "bamboo.result.quarantine"]/]
[/#if]
    ...
    AJS.$(function ($) {
        if (Piecon.browserSupportsDynamicFavicon()) {
            BAMBOO.FaviconUpdate({
                                     resourcePrefix: '[@cp.getStaticResourcePrefix /]',
                                     faviconRestUrl: '[@ww.url value="/rest/api/latest/plan/favicon/"/]${immutablePlan.planKey}'
            }).init();
        }
    });
</script>
