...
<li id="b${chainResultKey}" class="${cssClass}">
    ...
    <a id="stopBuild_${chainResultKey}" href="[@ww.url namespace='/build/admin/ajax' action='stopPlan' /]?planResultKey=${chainResultKey}" class="build-stop">[@ui.icon type="build-stop" text=cancelBuildText /]</a>
${buildMessage}
    ...
    [#if stages?has_content]
        ...
        <script type="text/javascript">
            ...
            var updateStage = function (data) {
                var $stageTitle = AJS.$(
                    "#stage_" + data.stage.id + " > div > dl > dt"
                );

                var newTitleHTML = data.stage.description ?
                    (data.stage.name + " <span>" + data.stage.description + "</span>") : data.stage.name;
                ...
                // update newTitle on all create-job links
                if (data.stage.name) {
                    var existingStage = AJS.$('.create-job a');
                    var existingStageParam = 'existingStage=';
                    var existingStageLocation = existingStage.attr('href').split(existingStageParam, 1);

                    existingStage.attr('href', [
                        existingStageLocation[0],
                        AJS.$('<div/>').text(data.stage.name).html()
                    ].join(existingStageParam));
                }
            };
                                ...
                                <li id="job-${job.key}" [#if job.description?has_content] title="${job.description}"[/#if] class="[#if (job.suspendedFromBuilding)!false]disabled [/#if][#if job.key == currentKey]active [/#if][#if job.status??]${job.status.displayClass}[/#if]" data-job-key="${job.key}">
                                    [#if job.status?has_content]
                                        ...
                                        ${soy.render("widget.icons.statusIcon", {
                                            "status":  job.status.displayClass
                                        })}
...
[#macro createJobListItem chain stage]
    [#if fn.hasPlanPermission("ADMINISTRATION", chain)]
        [#import "../widget/action-tip/action-tip.ftl" as actionTip]
        <li class="create-job">
            <a id="createJob_${stage.id}"
                href="[@ww.url action='addJob' namespace='/chain/admin' buildKey=chain.key existingStage=stage.name /]"
                title="[@ww.text name='job.create.add.header.title' /]"
            >
                [@ui.icon type="add" useIconFont=true /][@ww.text name='job.create.add' /]
            </a>
            [@ww.text id="contentAction" name="action.tip.job.add"]
                [@ww.param][@help.href pageKey="agent.remote.capabilities"/][/@ww.param]
            [/@ww.text]
            [@actionTip.addActionTip
                triggerSelector="#createJob_${stage.id}"
