...
[#macro tooltip target text='' showDelay=500 width=300 addMarker=false url='']
    ...
    [#if tooltipText?has_content || url?has_content]
        ${soy.render("bamboo.widget.tooltip.tooltip", {
            'trigger': '#${target}'
            ,'url': '${url?js_string}'
            ,'content': '${tooltipText?js_string}'
            ,'width': width
            ,'showDelay': showDelay
            ,'addMarker': addMarker
        })}
...
[#macro cronBuilder name idPrefix="" helpKey=""]
    ...
    <script type="text/javascript">
        AJS.$("#${displayFieldId}").cronBuilder({
             ...
             dialogTrigger: '<a id="${idPrefix}cronBuilder" class="cron-builder">[@ui.icon useIconFont=true type="edit" textKey="cronEditorBean.title"/]</a>',
