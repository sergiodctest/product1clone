...
[#macro displayTestSummary testResults testSummary displayIgnored=false showJob=false]
[#-- @ftlvariable name="testResults" type="com.atlassian.bamboo.resultsummary.tests.FilteredTestResults" --]
[#-- @ftlvariable name="testSummary" type="com.atlassian.bamboo.resultsummary.tests.TestResultsSummary" --]
    ...
    [#if testSummary.fixedTestCaseCount > 0]
        [@testsTable id="fixed-tests" testsMap=testResults.fixedTests
            showQuarantineActions=false
            showReason=true showJob=showJob showDuration=true showFailingSince=true
            caption=testsTableCaption('buildResult.tests.summary.fixed.title', testSummary.fixedTestCaseCount) /]
    [/#if]

    [#if displayIgnored]
        [#-- anchor for the links from results summary screen --]
        <a name="ignored-tests"></a>

        [#if displayIgnored && testSummary.hasQuarantinedTestResults()]
            [@testsTable id="quarantined-tests" testsMap=testResults.quarantinedTests
                showQuarantineInfo=true showReason=true showJob=showJob showDuration=true showFailingSince=true
                showStackTrace=true isStackTraceExpanded=true
                caption=testsTableCaption('buildResult.tests.summary.quarantined.title', testSummary.quarantinedTestCaseCount) /]
        [/#if]

        [#-- need to check for the content because skipped test counter in TestResultsSummary is not updated for quarantined tests --]
        [#-- showStackTrace is true just to keep the columns aligned --]
        [#if displayIgnored && testResults.skippedTestList?has_content]
            [@testsTable id="skipped-tests" cssClass="skipped" testsMap=testResults.skippedTests
                showQuarantineInfo=false showQuarantineActions=false showReason=true showJob=showJob showDuration=false showFailingSince=true
                showStackTrace=false isStackTraceExpanded=false
                caption=testsTableCaption('buildResult.tests.summary.skipped.title', testResults.skippedTestList?size) /]
        [/#if]
    [/#if]
[/#macro]
...
[#macro testsTable testsMap showReason showJob showDuration showFailingSince=false showStackTrace=false isStackTraceExpanded=false
        showQuarantineInfo=false showQuarantineActions=true id="" cssClass="" caption=""]
[#-- @ftlvariable name="testsMap" type="com.google.common.collect.Multimap<com.atlassian.bamboo.resultsummary.tests.TestClassResultDescriptor, com.atlassian.bamboo.resultsummary.tests.TestCaseResult>" --]
    [#assign testIndex = 0]
    [#assign colspan = 2]
    [#if showStackTrace]
            ...
            <tbody>
                [#list testsMap.keySet() as testResultClass]
                    [#list testsMap.get(testResultClass) as testResult]
                        ...
                        <tr id="testCase-${testResult.testCase.id}" class="test-case ${testResult.state.displayName?lower_case?html}[#if (testIndex) % 2 == 0] zebra[/#if] test-case-row-${testResult.testCase.id}[#if showStackTrace && hasStackTrace] [#if shouldExpandStackTrace]expanded[#else]collapsed[/#if][/#if]">
                            ...
                            <td class="status">
                                ${soy.render("widget.icons.statusIcon", {
                                    "status": testResult.state.displayName?lower_case?html,
                                    "text": testResult.state.displayName?html
                                })}
                            </td>
                            <td class="test">
                            ...
                            [#if action.getUser()?? &&  fn.hasPlanPermission("ADMINISTRATION", immutablePlan)]
                                <td class="actions">
                                    ...
                                    [@ui.standardMenu triggerText="" id="${id}-testResultActions-${testResult.testCase.id}" icon="configure" useIconFont=true subtle=true]
                                        ...
                                        [#if showQuarantineActions && action.testQuarantineEnabled]
                                            [@displayTestQuarantineActions testResult=testResult plan=immutablePlan /]
                                        [/#if]
...
[#macro displayTestToJiraActions linkedJiraIssueKey testCaseId planResultKey returnUrl ]
    ...
    [@ui.displayLink
            ...
            inList=true
            cssClass=buttonCssClass
            extraAttributes={"data-test-case-id":testCaseId} /]
[/#macro]

[#macro displayTestQuarantineActions testResult plan]
[#-- @ftlvariable name="testResult" type="com.atlassian.bamboo.resultsummary.tests.TestCaseResult" --]
[#-- @ftlvariable name="plan" type="com.atlassian.bamboo.plan.cache.ImmutablePlan" --]
        ...
        [#if !featureManager.limitedTimeQuarantineEnabled]
            [#local extraAttributes={
                ...
                "data-test-id": testResult.testCase.id
            }/]
        [#else]
            [#local extraAttributes={
                "data-plan-key": plan.key,
                "data-test-id": testResult.testCase.id,
                "data-expiry-overridable": action.testQuarantineExpiryOverridable?string,
                "data-expiry-duration": action.testQuarantineExpiryDuration,
                "data-expiry-period": action.testQuarantineExpiryPeriod
            }/]
        [/#if]
        [@ui.displayLink
            ...
            id="toggleTestIgnore-${testResult.testCase.id}"
            mutative=true
        /]
[/#macro]
...
[#-- @ftlvariable name="testSummary" type="com.atlassian.bamboo.resultsummary.tests.TestResultsSummary" --]
    ...
    [#assign hasIgnoredTests=testSummary.hasIgnoredTestResults() /]
    <ul id="testsSummary">
        ...
        [#if hasIgnoredTests]
            <li id="testsSummaryIgnored">
                ...
                [@ww.text name='buildResult.tests.summary.ignored.summary']
                    [@ww.param name="value" value=testSummary.ignoredTestCaseCount /]
                [/@ww.text]
            </li>
        [/#if]
