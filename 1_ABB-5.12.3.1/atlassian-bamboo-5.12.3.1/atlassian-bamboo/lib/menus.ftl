...
[#macro displayTabbedContent selectedTab location admin=false historyXhrDisabled=false sidebar="" linkCssClass=""]
<div class="aui-tabs horizontal-tabs aui-tabs-disabled[#if historyXhrDisabled] history-xhr-disabled[/#if]">
    ...
    <ul class="tabs-menu">
        [#if admin]
            [@displayTabsNoAction selectedTab=selectedTab location=location linkCssClass=linkCssClass/]
        [#else]
            [@displayTabs selectedTab=selectedTab location=location linkCssClass=linkCssClass/]
        [/#if]
    </ul>
...
[#macro displayTabs selectedTab location innerLocation="" linkCssClass=""]
    ...
    [#list action.getWebSectionsForLocation(location) as section ]
        ...
        <li data-section="${section.key}" class="menu-item[#if selectedTab == item.name] active-tab[/#if]">
            <a [#if id?has_content] id="${id}"[/#if] href="${url}" class="${linkCssClass}"><strong>${item.webLabel.displayableLabel}</strong></a>
        </li>
...
[#macro displayTabsNoAction selectedTab location linkCssClass=""]
    [#assign selectedIndex = 0/]
    ...
    [#list ctx.getWebSectionsForLocationNoAction(location, req) as section ]
        [#list ctx.getWebItemsForSectionNoAction(location + '/' + section.key, req) as item]
            ...
            [/#if]
        <li class="menu-item[#if selectedTab == item.name] active-tab[/#if]">
            <a [#if id??] id="${id}"[/#if] href="${url}" class="${linkCssClass}"><strong>${item.webLabel.displayableLabel}</strong></a>
        </li>
        [/#list]
    [/#list]
[/#macro]
    ...
    [#if fn.hasPlanPermission('BUILD', plan)]
        ...
        [#if numberOfRunningChains > 0]
            [#if numberOfRunningChains > 1]
                [@ww.url id="viewRunningPlansUrl" action="viewRunningPlans" namespace="/ajax" planKey=plan.key returnUrl=currentUrl/]
