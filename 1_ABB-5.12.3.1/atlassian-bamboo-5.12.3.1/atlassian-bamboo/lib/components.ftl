    ...
    <div id="plan-errors">
        [#if fn.hasAdminPermission()]
            <div class="floating-toolbar">
                <a class="aui-button mutative" href="[@ww.url value='/chain/removePlanErrorsFromLog.action?' planKey=planKey buildNumber=buildNumber returnUrl=currentUrl /]">[@ww.text name="plan.error.bulk.delete"/]</a>
            </div>
        [/#if]
...
[#-- @ftlvariable name="error" type="com.atlassian.bamboo.logger.DecoratedErrorDetailsImpl" --]
    ...
    [#if error.throwableDetails??]
        <div class="footer">
            [@ww.url id='viewErrorUrl' value='/admin/viewError.action'
    ...
    [/#if]
    [#if fn.hasAdminPermission() ]
        [@s.url id='removeErrorFromLogUrl' namespace='/' action='removeErrorFromLog'
            buildKey=error.buildKey
            error=error.errorNumber
...
[#macro displayManualVariables result]
    [#local manualVariables=result.manuallyOverriddenVariables]
...
[#macro captcha]
    ...
    <div class="field-group">
        <img id="captcha-image" class="captcha-image" src="${req.contextPath}/captcha?atl_token=${ctx.xsrfToken}"/>
...
[#macro displayLinkButton buttonId buttonLabel buttonUrl='' iconClass='' altText="" altTextKey="" cssClass="" primary=false disabled=false mutative=false]
