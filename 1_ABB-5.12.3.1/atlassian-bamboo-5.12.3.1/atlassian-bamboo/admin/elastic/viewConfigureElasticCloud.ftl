...
[#if featureManager.onDemandInstance]
    [@ww.text name="elastic.configure.blurb.ondemand" /]
[#else ]
    [@ww.text name="elastic.configure.blurb.btf" /]
[/#if]
</p>
...
[#if elasticConfig?? && elasticConfig.enabled]
    ...
    <div id="elastic-bamboo-configuration-view">
        ...
        [@ui.bambooInfoDisplay titleKey="elastic.configure.agents.title"]
            <p>
            [#if featureManager.onDemandInstance]
                [@ww.text name="elastic.configure.agents.description.ondemand" /]
            [#else ]
                [@ww.text name="elastic.configure.agents.description.btf" /]
            [/#if]
            </p>
            [@ww.label labelKey='elastic.configure.agents.maximum.number.of.elastic.agents' value=elasticConfig.maxConcurrentInstances /]
            [#if featureManager.onDemandInstance]
                [@ww.label labelKey='elastic.configure.agents.license.tier' escape=false]
                    [@ww.param name='value']
                        ${licenseTier}
                        [@help.url pageKey="elastic.cost" cssClass="pricing-details"][@ww.text name="elastic.configure.agents.pricing.details"/][/@help.url]
                    [/@ww.param]
                [/@ww.label]
            [/#if]
    ...
    [@ww.form
        ...
        [@ui.bambooInfoDisplay titleKey="elastic.configure.agents.title"]
            <p>
                ...
                [#else]
                    [#if featureManager.onDemandInstance]
                        [@ww.text name='elastic.configure.nonexistent.ondemand' /]
                    [#else ]
                        [@ww.text name='elastic.configure.nonexistent.btf' /]
                    [/#if]
                [/#if]
            </p>
        [/@ui.bambooInfoDisplay]
