...
[#macro displayElasticInstanceId instanceId, instance='' ]
    [#if instance?has_content && permissionManager.canManageElasticBamboo()]
...
[#macro elasticImageConfigurationViewProperties configuration displayConfigurationUrl=false short=false ]
    ...
    [@ww.label labelKey='elastic.image.configuration.availabilityZone.preferred' ]
        [@ww.param name="value"]
            [#if configuration.availabilityZones?has_content]
                ${fn.join(configuration.availabilityZones)}
            [#else]
...
[#macro editElasticInstance mode configuration='']
    [#-- @ftlvariable name="configuration" type="com.atlassian.bamboo.agent.elastic.server.ElasticImageConfiguration" --]

    [#if mode == "edit"]
        [#assign action="saveElasticImageConfiguration" /]
        [#assign cancelUri=returnUrl /]
    [#else]
        [#assign action="createElasticImageConfiguration" /]
        [#assign cancelUri="viewElasticImageConfigurations.action" /]
    [/#if]
    ...
    [@s.form action="${action}"
        ...
        [@ui.bambooSection dependsOn='ebsEnabled' showOn=true]
            ...
            [@s.checkbox key="elastic.image.configuration.legacy.ebs" name="legacyEbsHandlingEnabled"/]
        [/@ui.bambooSection]

        [@s.select name='instanceTypes' list=availableInstanceTypes listKey='name()' listValue='name' key='elastic.instance.type' multiple=false size=6 cssClass="long-field"/]

        [@s.checkbox key='elastic.instance.vpc.use' name="vpcEnabled" toggle=true/]
        [@ui.bambooSection dependsOn='vpcEnabled' showOn=true]
            [#if subnetMap?has_content]
                [@s.select name='subnets' list=subnetMap listKey='value' listValue='key' key='elastic.instance.vpc.subnet' multiple=true size=subnetMap?size cssClass="long-field" toggle=true/]
            [/#if]
        [/@ui.bambooSection]

        [@ui.bambooSection dependsOn='vpcEnabled' showOn=false]
        ...
        [@s.select name='product' list=products labelKey='elastic.instance.product'/]

        [#if configuration?has_content]
            [@s.textfield key="agent.capability.type.system.agent.environment.iam.instance.profile" name="iamInstanceProfile"/]
            [@s.checkbox key="agent.capability.type.system.agent.environment.ebs.optimised" name="ebsOptimised"/]
            [@s.label key="elastic.image.configuration.architecture" value=configuration.architecture /]
            [@s.label key="elastic.image.configuration.region" value=configuration.region /]
            [@s.label key="elastic.image.configuration.rootDeviceType" value=configuration.rootDeviceType /]
            [#if configuration.virtualisationType??]
                [@s.label key="elastic.image.configuration.virtualisationType" value=configuration.virtualisationType /]
            [/#if]
...
[#-- @ftlvariable name="currentSpotPrices" type="com.atlassian.aws.ec2.SpotPrices" --]
    ...
    <tbody>
        [#list instanceTypes as instanceType]
            ...
            <tr>
                ...
                [#list currentSpotPrices.getProducts() as product]
                    [#assign priceRange = currentSpotPrices.get(product, awsInstanceType)!"n/a"/]
                    ...
                    <td>
                        ...
                        [#if editMode]
                            [@s.textfield name='${getBidFieldName(product, awsInstanceType)}' value="${bid}" labelKey="elastic.configure.spot.instances.bid" cssClass="short-field" /]
                        [#else]
                            [#if bid?has_content]
                                <div>[@s.text name='elastic.configure.spot.instances.bid' /]${bid}</div>
                            [#else]
                                <div class="subGrey">[@s.text name='elastic.configure.spot.instances.no.bid' /]</div>
                            [/#if]
                        [/#if]
                        [#if !bid?has_content]<span class="subGrey">[/#if]

                        [#if priceRange!="n/a"]
                            [@s.text name="elastic.configure.spot.instances.price.timestamp" id="priceTimestamp"]
                                [@ww.param]${priceRange.left().timestamp?datetime?string}[/@ww.param]
                            [/@s.text]
                            <span title="${priceTimestamp}">[@s.text name="elastic.configure.spot.instances.price" /] $${priceRange.left()}-$${priceRange.right()}</span>
                        [#else]
                            [@s.text name="elastic.configure.spot.instances.price" /]
                            <span class="subGrey">${priceRange}</span>
                        [/#if]
...
[#macro sshAccess sshEnabled pkHttpLinkAndLocalLocation hostname]
    [#if sshEnabled ]
        [@ui.bambooSection titleKey='elastic.instance.ssh']
            [@s.text name='elastic.instance.ssh.ok.description']
                [@s.param]${pkHttpLinkAndLocalLocation}[/@s.param]
                [@s.param]${hostname?html}[/@s.param]
            [/@s.text]
        [/@ui.bambooSection]
        [@ui.bambooSection titleKey='elastic.instance.logs']
            [@ww.text name='elastic.instance.logs.description']
                [@ww.param]${pkHttpLinkAndLocalLocation}[/@ww.param]
                [@ww.param]${hostname?html}[/@ww.param]
            [/@ww.text]
        [/@ui.bambooSection]
    [/#if]
[/#macro]