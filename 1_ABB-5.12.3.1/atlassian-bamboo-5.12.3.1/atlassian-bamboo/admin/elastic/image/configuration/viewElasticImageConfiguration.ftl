...
<body>
    ...
    <h1>
 	   <a href="${req.contextPath}/admin/elastic/image/configuration/viewElasticImageConfigurations.action" id="elasticConfigurationsLink" class="mutative" title="[@ww.text name='elastic.image.configuration.list.heading' /]">[#t]
 	       [@ww.text name='elastic.image.configuration.list.heading' /]</a>&nbsp;&rsaquo;&nbsp;${configuration.osName!""} ${configuration.configurationName?html} [#if configuration.dedicated] [@ui.agentDedicatedLozenge/][/#if][#t]
 	</h1>
    ...
    [@dj.tabContainer headings=tabs selectedTab='${selectedTab!}']
        [@dj.contentPane labelKey='elastic.image.configuration.capabilities']
            [@ww.url id='reloadUrl' action='reloadDefaultCapabilities' namespace='/admin/elastic' configurationId=configuration.id returnUrl=currentUrl /]
            [@ui.bambooPanel titleKey='elastic.image.configuration.capabilities' descriptionKey='elastic.image.configuration.capabilities.description'
                tools='<a href="#addCapability" onclick="">${action.getText("agent.capability.add")}</a> | <a href="${reloadUrl}" class="requireConfirmation mutative" title="${action.getText("elastic.image.configuration.capabilities.revert")}">${action.getText("elastic.image.configuration.capabilities.revert")}</a>'
            ]
                [#if configuration.capabilitySet?? && configuration.capabilitySet.capabilities?has_content]
