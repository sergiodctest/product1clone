...
[@ww.form action=runAction namespace="/ajax" submitLabelKey="global.buttons.run" cssClass="custom-build"]
    ...
    [#assign noFields=true /]

    [#if !continuable && action.planHasRepository()]
        [#assign noFields=false /]
    ...
    [#if action.overridableVariables?has_content]
        [#assign noFields=false /]
        [@displayFieldGroup labelKey='build.editParameterisedManualBuild.variables']
            ${soy.render("bamboo.feature.variables.variablesTable", {
                "showOverrideFooter": true,
                "userCanOverrideVariables": action.userCanOverrideVariables,
                "hideAddHeader": true,
                "inlineEdit": false,
            ...
            <script type="text/javascript">
                require(['jquery', 'feature/variables'], function($, variables){
    ...
    [#if noFields]
        [@ui.messageBox type="info" titleKey="build.editParameterisedManualBuild.nothingToCustomise" /]
    [/#if]
[/@ww.form]
