...
[#include "/fragments/storage/storageCappingBanner.ftl"]
...
[#include "/fragments/storage/storageCappingDialogs.ftl"]
...
[#function headerItems headerSection activeKey=""]
    ...
    [#local output]
        [#list ctx.getWebItemsForSectionNoAction(headerSection, req) as webItem]
            ...
            [/#if]
            <li id="system_${webItem.key.replace('.','_')}" [#if activeKey == webItem.key] class="selected"[/#if]>[#t]
                [#if dropdownWebSections?size gt 0]
                    ${soy.render("aui.dropdown2.dropdown2", {
                        "trigger": {
                            "id": (webItem.link.getRenderedId(webFragmentsContext))!"",
                            "content": triggerContents!'',
                            "text": triggerText!'',
                            "extraClasses": triggerClass!'',
                            "title" : webItem.webLabel.getDisplayableLabel(req, webFragmentsContext),
                            "showIcon": false
                        },
                        "menu": {
...
[#function primaryHeaderItems activeKey=""]
    [#local output]
        [#if (fn.hasGlobalPermission('CREATE') || fn.displayCreateRepositoryLink()) && !ctx.ec2ConfigurationWarningRequired]
...
[#function secondaryHeaderItems activeKey=""]
    [#local quickNav]
        [#if !decoratorName?? || decoratorName != "login"]
            <li>
                <div id="quicksearch" class="aui-quicksearch">
                    <input name="term" class="term text" type="text" autocomplete="off" accesskey="/" placeholder="[@ww.text name="global.buttons.search" /]" title="[@ww.text name="menu.quicksearch.tooltip" /]">
    ...
    [#local output]
        ...
        [/#if]
    [/#local]
    [#return renderHeaderItemContainer(quickNav + headerItems("header.global.secondary", activeKey) + output) /]
[/#function]

[#function renderWebItem webItem]
    [#local webFragmentsContext = ctx.getWebFragmentsContextMapNoAction(req) /]
    [#local id = webItem.link.getRenderedId(webFragmentsContext)! /]
    [#local accesskey = webItem.link.getAccessKey(webFragmentsContext)! /]
    [#local relAttrValue = webItem.params.get("rel")! /]
    [#local output][#rt]
        <a href="${webItem.link.getDisplayableUrl(req, webFragmentsContext)}"[#if id?has_content] id="${id}"[/#if][#if accesskey?has_content] accesskey="${accesskey}"[/#if] [#if relAttrValue?has_content] rel="${relAttrValue}"[/#if]>${webItem.webLabel.displayableLabel}</a>[#t]
    [/#local][#lt]
    [#return output /]
[/#function]
