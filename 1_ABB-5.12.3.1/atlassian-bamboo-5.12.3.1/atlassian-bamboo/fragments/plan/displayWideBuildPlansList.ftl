...
[#macro planOperations build]
    ...
    [#if fn.hasPlanPermission('WRITE', build)]
        <a id="editBuild:${build.key}" href="${fn.getPlanEditLink(build)}" >[@ui.icon useIconFont=true type="edit" text="Edit '${build.name}'"/]</a>
    [/#if]
            ...
            <td class="build">
                <a id="viewBuild:${build.key}" href="${req.contextPath}/browse/${build.key}"[#if build.description?has_content] title="${build.description!?html}"[/#if]>[#if showBranchIcons && build.master??][@ui.icon type="devtools-branch" useIconFont=true /][/#if]${build.buildName}</a>
                    ...
                    ${soy.render("bamboo.deployments:linked-deployment-shortcut", 'feature.plan.linkedDeployment.linkedDeploymentDashboardDropdown', {
                        ...
                        'hasLinkedDeployments': ctx.hasLinkedDeployments(((build.parent)!build).planKey)
                    })}
            </td>
...
[#macro disableAllQuickFiltersLink]
    [@s.text name="quick.filters.dashboard.change.or.disable"]
        [@s.param]quick-filters-disable-all[/@s.param]
    [/@s.text]
[/#macro]
            ...
            <td colspan="100" data-last-project-key="${plansForDashboard.lastProjectKey!}"[#if plansForDashboard.showMoreRequired] class="show-more-plans"[/#if]> [#--Apparently if the colspan is a bit big, it doesn't matter--]
                <div>
                    ...
                    [#if !plansForDashboard.emptyResults]
                        [#if plansForDashboard.showMoreRequired]
                            <button class="show-more-trigger [#if action.anyQuickFilterActive]quick-filters[/#if]">
                                [@ww.text name="dashboard.filter.showMore"/]
                            </button>
                        [/#if]
                        ...
                        [#if plansForDashboard.notIncludedCount > 0]
                            [#if action.dashboardFilterEnabled]
                                <p class="show-more-text">
                                    [@s.text name='dashboard.filter.on.message']
                                        [@s.param]${plansForDashboard.notIncludedCount}[/@s.param]
                                        [@s.param]${editFilterByProjects}[/@s.param]
                                    [/@s.text]
                                </p>
                            [#elseif action.anyQuickFilterActive]
                                <p class="show-more-text">
                                    [@s.text name='quick.filters.dashboard.plans.hidden']
                                        [@s.param value=plansForDashboard.notIncludedCount/]
                                    [/@s.text]
                                </p>
                                <p class="show-more-text">[@disableAllQuickFiltersLink /]</p>
                            [/#if]
                        [/#if]
                        [#-- Show Filter this hint when no filter + lots of plans --]
                        [#if !action.dashboardFilterEnabled && !action.anyQuickFilterActive && plansForDashboard.multiPage && user??]
                            <p class="show-more-text">
                    ...
                    [#else]
                        [#if action.dashboardFilterEnabled]
                            ...
                            <p>
                                [@ww.text name='dashboard.filter.noPlans.help']
                                    [@ww.param]${editFilterByProjects}[/@ww.param]
                                    [@ww.param][@ww.url action='toggleDashboardFilter.action' namespace='ajax' filterEnabled=(!dashboardFilterEnabled)?string returnUrl=currentUrl /][/@ww.param]
                                [/@ww.text]
                            </p>
                        [#elseif action.anyQuickFilterActive]
                            <p>[@s.text name='quick.filters.dashboard.plans.no.match' /]</p>
                            <p>[@disableAllQuickFiltersLink /]</p>
                        [/#if]
                    [/#if]
                </div>
            </td>
    ...
    <script type="text/javascript">
        ...
        AJS.$(function($) {
            $(".stopMultipleBuilds").each(function() {
                ...
                simpleDialogForm($this, "[@ww.url action='viewRunningPlans.action' namespace='ajax' returnUrl=currentUrl /]&planKey=" + planKey,
                                 800, 400,
