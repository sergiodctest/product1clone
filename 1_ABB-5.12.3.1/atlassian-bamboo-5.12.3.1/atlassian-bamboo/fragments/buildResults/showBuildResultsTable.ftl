...
[#-- @ftlvariable name="buildResult" type="com.atlassian.bamboo.resultsummary.ResultsSummary" --]
                <td class="${buildResult.buildState}">
                    [#if showFullBuildName]
                        ${soy.render("widget.icons.statusIcon", {
                            "status": "${fn.getPlanStatusIcon(buildResult)?lower_case}"
                        })}
                    ...
                    [#else]
                        ...
                        [/#if]
                        <a class="statusIndicator" id="buildResult_${buildResult.planResultKey}" href="${req.contextPath}/browse/${buildResult.planResultKey}" title="${title}">
                            ${soy.render("widget.icons.statusIcon", {
                                "status": "${fn.getPlanStatusIcon(buildResult)}"
                            })}
                    ...
                    [#if artifactLinks?has_content]
                        ...
                        [#list artifactLinks as artifactLink]
                            ...
                            [#if artifactUrl?has_content]
                                <div><a href="${req.contextPath}${artifactUrl}">${artifactLink.label?html}</a></div>
