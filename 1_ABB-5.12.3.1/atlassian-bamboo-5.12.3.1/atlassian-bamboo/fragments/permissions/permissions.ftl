[#-- @ftlvariable name="" type="com.atlassian.bamboo.configuration.AbstractPermissionAction" --]
...
[#macro permissionCheckBox principal permission type='user' ]
    [#local permissionkey='bambooPermission_${type}_${principal?html}_${permission}' /]
    [#local granted=grantedPermissions.contains(permissionkey) /]
    <td class="checkboxCell clickable">
        <input type="checkbox" name="grantedPermissions" value="${permissionkey}" [#if granted]checked[/#if] />
    </td>
[/#macro]

[#macro permissionsEditor editablePermissions editablePermissionsForLoggedInUsers={} entityId=0 anonymousAllowed=true]

    [#if !(editablePermissionsForLoggedInUsers?has_content)]
        [#local editablePermissionsForLoggedInUsers=editablePermissions /]
    [/#if]
    ...
    [#if entityId > 0]
        [@ui.displayButtonContainer secondary=true]
            ...
            [@cp.displayLinkButton buttonId='createUserPrincipalButton' buttonLabel='build.permission.add.user.title' buttonUrl=addUserPrincipal cssClass='add-principal' /]

            [@s.url id='addGroupPrincipal' value='/ajax/addPermissionPrincipal.action?principalType=Group&entityId=${entityId}' returnUrl=currentUrl/]
            [@cp.displayLinkButton buttonId='createGroupPrincipalButton' buttonLabel='build.permission.add.group.title' buttonUrl=addGroupPrincipal cssClass='add-principal' /]
    ...
    <table class="aui permissions" id="configureBuildOtherPermissions">
        ...
        <tbody>
            <tr>
                ...
                [#list editablePermissions.keySet() as key]
                    [#if editablePermissionsForLoggedInUsers[key]??]
                        [@permissionCheckBox principal='ROLE_USER' type='role' permission=editablePermissionsForLoggedInUsers[key] /]
                    [#else ]
                        <td></td>
                    [/#if]
                [/#list]
