            ...
            [#list executableAgentsMatrix.imageMatches as image]
                <li>
                    [#if permissionManager.canManageElasticBamboo()]
                        <a href="[@ww.url action='viewElasticImageConfiguration' namespace='/admin/elastic/image/configuration' configurationId='${image.id}' /]">${image.amiId?html} (${image.configurationName?html})</a>
                    [#else]
