                    ...
                    [#switch agent.type.identifier]
                        [#case "local"]
                            [#if buildAgent.active && buildAgent.agentStatus.allowDelete]
                                ...
                                [@cp.displayLinkButton buttonId="" buttonLabel="agent.stop" buttonUrl=stopAgentUrl altTextKey='agent.stop.description' mutative=true /]
                            [#elseif !buildAgent.active]
                                [@ww.url action='startAgent' namespace='/admin/agent' agentId='${agentId}' id='startAgentUrl' /]
                                [@cp.displayLinkButton buttonId="" buttonLabel="agent.start" buttonUrl=startAgentUrl mutative=true /]
                            [/#if]
                        [#-- no break here --]
                        [#case "remote"]
...
<img src="${req.contextPath}${currentAgentStatus.imagePath}" alt="${currentAgentStatus.label?html}"/>
    ...
    [#else]
    ${currentAgentStatus.label?html}[#t]
    [/#if]
    [#if currentAgentStatus.url??]
    </a> [#lt]
    [/#if]
    ...
    [#list capabilitySetDecorator.groups as group]
        ...
        <table id="capabilities-${group.typeKey}" class="capabilities aui">
            ...
            <tr>
                ...
                <th class="operations">[@s.text name='global.heading.operations'/]</th>
            </tr>
            </thead>
            <tbody>
                [#list group.decoratedObjects as capability]
                <tr[#if capabilityKey?? && capabilityKey == capability.key] class="selectedRow"[/#if]>
                    ...
                    <td class="operations">
                        [@showCapabilityOperations capability=capability agent=agent elasticImageConfiguration=elasticImageConfiguration showView=true showEdit=(showEdit && !capability.readonly) showDelete=(showDelete && !capability.readonly) returnAfterOpUrl=returnAfterOpUrl capabilityName='${capability.label}'/]
                    </td>
                </tr>
                [/#list]
            </tbody>
...
[#macro showName capability]
    ...
    [#if capability.extraInfo?has_content]
        <span class="subGrey">(${capability.extraInfo?html})</span>
    [/#if]
[/#macro]
...
[#macro showRequirementOperations requirement build returnUrl='']
    ...
    <a href="[@ww.url action='editBuildRequirement' namespace='build/admin/edit' buildKey=build.key requirementId=requirement.id?url][#if returnUrl?has_content][@ww.param name='returnUrl']${returnUrl}[/@ww.param][/#if][/@ww.url]"
            class="edit-requirements" title="[@ww.text name='agent.requirement.edit.description' /]">
        [@ww.text name='global.buttons.edit' /]
    </a>

    <a href="[@ww.url action='deleteBuildRequirement' namespace='/build/admin/edit' buildKey=build.key requirementId=requirement.id?url][#if returnUrl?has_content][@ww.param name='returnUrl']${returnUrl}[/@ww.param][/#if][/@ww.url]"
            class="requireConfirmation mutative" title="[@ww.text name='agent.requirement.delete.description'][@ww.param]${requirement.label}[/@ww.param][/@ww.text]">
            ...
            [#if agent?has_content]
                ...
                [#if showDelete]
                    ...
                    <a id="delete:${capability.key?html}From${agent.name?html}"
                       ...
                       title="[@ww.text name='agent.capability.delete.description'][@ww.param]${capabilityName?html}[/@ww.param][/@ww.text]">[@ww.text name='global.buttons.delete' /]</a>
                [/#if]
...
[#macro displayEditCapabilityFields]
    ...
    [@ww.textfield labelKey='agent.capability.type.${capabilityType}.value' name='capabilityValue' cssClass='long-field' /]
[/#macro]
...
[#--The Data Table--]
    [#if onlineRemoteAgents?has_content]
        [@ww.form action="configureAgents!reconfigure.action" id="remoteAgentConfiguration" theme="simple"]
            ...
            [/#if]
        <table id="online-remote-agents" class="aui">
            <colgroup>
                [#if showOperations]
                    <col width="5"/>
                [/#if]
            ...
            <tbody>
                ...
                <tr class="agent${agent.agentStatus}">
                    ...
                    [/#if]
                    [#assign dedicatedNoMatch = dedicatedNonMatchingAgents?has_content && dedicatedNonMatchingAgents?seq_contains(agent) /]
                    <td>[@ui.renderAgentNameAdminLink agent dedicatedNoMatch/]</td>
...
[#macro systemInfoAgent systemInfo='']
    [#if systemInfo?has_content]
        [@ui.bambooPanel]
        ...
        [/@ui.bambooPanel]
    [#else]
    <p>[@ww.text name='system.info.not.found' /]</p>
    [/#if]
[/#macro]

[#macro capabilitiesElasticAgent capabilitySet capabilitySetDecorator]
