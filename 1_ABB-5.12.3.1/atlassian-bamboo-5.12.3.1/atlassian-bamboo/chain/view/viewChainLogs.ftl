...
<html>
<head>
	[@ui.header page='Logs' object='${immutablePlan.key} ${(resultsSummary.buildNumber)!}' title=true /]
            ...
            <tbody>
                ...
                [#list resultsSummary.stageResults as stageResult]
                    [#list stageResult.sortedBuildResults as buildResult]
                        [#local logsExpired = buildResult.logSize?has_content && buildResult.logSize == -1 /]
                        [#local shouldExpand = !logsExpired && (buildResult.failed || singleJobPlan) /]
                        ...
                        <tr class="${shouldExpand?string('expanded', 'collapsed')}">
                            ...
                            [#if !logsExpired && (buildResult.finished || buildResult.inProgress)]
                                <td>
