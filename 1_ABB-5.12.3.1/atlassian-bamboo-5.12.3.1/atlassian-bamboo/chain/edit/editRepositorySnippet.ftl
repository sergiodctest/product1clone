...
[@ww.form action=submitAction
    ...
    [#if repositoryId > 0 && !repositoryDefinition.global && fn.hasGlobalPermission('CREATEREPOSITORY')]
    ...
    [#if selectableRepositories?has_content]
    ...
    [/@ui.bambooSection]
    [#else]
        <div>[@s.text name="repository.add.no.linked"/]</div>
    [/#if]
