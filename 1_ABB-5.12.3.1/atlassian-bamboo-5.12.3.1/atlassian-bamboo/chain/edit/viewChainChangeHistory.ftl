...
<body>
    [#if filterStart > 0 || filterEnd >0]
        [#assign descriptionText]
            ...
            [/@ww.text]
        [/#assign]
        [@ui.header pageKey="chain.auditLog.title" description=descriptionText id="audit_log_header" headerElement="h2" /]
    ...
    [#else]
        [@ui.header pageKey="chain.auditLog.title" descriptionKey="auditlog.configuration.changes" id="audit_log_header" headerElement="h2" /]
        [#if !enabled]
            [@ui.messageBox type="warning"]
                [@ww.text name='auditlog.global.disabled.warning'/]
            [/@ui.messageBox]
        [/#if]
        [#if pager.getPage()?? && fn.hasPlanPermission("ADMINISTRATION", plan)]
            <p>
