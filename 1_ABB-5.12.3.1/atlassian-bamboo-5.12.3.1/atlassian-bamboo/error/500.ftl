...
<body>
    ...
    [#if buildUtils?exists]
    <p>
        <b>Version:</b> ${buildUtils.getCurrentVersion()?if_exists}<br>
        <b>Build:</b> ${buildUtils.getCurrentBuildNumber()?if_exists}<br>
        <b>Build Date:</b> ${(buildUtils.getCurrentBuildDate()?date)?if_exists}
    </p>
    [/#if]
