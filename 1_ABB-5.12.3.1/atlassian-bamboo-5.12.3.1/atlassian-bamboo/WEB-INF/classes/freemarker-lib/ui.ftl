...
[#-- @ftlvariable name="stack" type="com.opensymphony.xwork2.ognl.OgnlValueStack" --]
...
<div class="form-view[#if cssClass?has_content] ${cssClass}[/#if]">
    [#if title?has_content || titleKey?has_content]
        [#if tools?has_content]
            <${toolsContainer} class="floating-toolbar" id="${titleKey}-tools">
            ${tools}
        </${toolsContainer}>
        [/#if]
...
[#-- ============================================================================================= @ui.bambooSection --]
[#macro bambooSection step=0 title='' titleKey='' description='' descriptionKey='' dependsOn='' showOn=true tools='' cssClass='' headerWeight='h3' toolsContainer='div' sectionContainer='fieldset' id='' collapsible=false isCollapsed=true]
    [#if collapsible]
    ...
    [#local summary]
        [#if step > 0 || title?has_content || titleKey?has_content]
            ...
            <${headerWeight}[#if collapsible] class="collapsible-header" tabindex="0"[/#if]>[#t/]
...
[#-- =============================================================================================== @ui.displayLink --]
[#macro displayLink href title="" titleKey="" icon='' iconPath='' id='' cssClass='' data='' extraAttributes={}
        inList=false showText=true showIcon=true useIconFont=false accessKey=''
        requiresConfirmation=false isAsynchronous=false data='' mutative=false]
    [#assign linkClass][#rt/]
    ...
    [#if inList]<li class="dropdown-item">[/#if][#t/]
        <a [#if !showText]title="${fn.resolveName(title, titleKey)}" [/#if]href="${href}" [#t/]
            [#if data?has_content] ${data}[/#if] [#t/]
            ...
            ${fn.renderExtraAttributes(extraAttributes)}[#t/]
        >[#t/]
            ...
            [#if showText]${fn.resolveName(title, titleKey)}[/#if][#t/]
        </a>[#t/]
    [#if inList]</li>[/#if][#t/]
[/#macro]
...
[#-- ================================================================================================= @ui.header--]
[#macro header object='' page='' pageKey='' description='' descriptionKey='' cssClass='' title=false showPlanSuspended=false headerElement='h1' id='']
    [#assign headerText]
    ...
    [#else]
        [#if headerText?trim?has_content]
        <${headerElement}[#if cssClass?has_content] class="${cssClass}"[/#if][#if id?has_content] id="${id}"[/#if]>${headerText?trim}</${headerElement}>[#t]
        [/#if]
        [#if description?has_content || descriptionKey?has_content]
            [@ui.displayText description descriptionKey /]
        [/#if]
...
[/#macro]
[#-- ================================================================================= @ui.agentDedicatedUnmatchedLozenge --]
[#macro agentDedicatedUnmatchedLozenge subtle=false]
    [@ui.lozenge colour="current" textKey="agent.dedicatedMissesRequirement" subtle=subtle/][#t]
[/#macro]
[#-- ================================================================================= @ui.renderAgentNameLink --]
[#--Please find equivalent soy methods in agent.soy--]

[#macro renderAgentNameLinkWithConsole agent immutablePlan]
    [#if ctx.featureManager.remoteAgentShellEnabled && fn.canEditPlan(immutablePlan)]
        [@ui.renderAgentNameLink agent=agent consolePermissionsPlanKey=immutablePlan.planKey/]
    [#else]
        [@ui.renderAgentNameLink agent=agent/]
    [/#if]
[/#macro]

[#macro renderAgentNameLink agent dedicatedLozenge=false consolePermissionsPlanKey='' missesRequirements=false]
    [#if consolePermissionsPlanKey?has_content]
        [#local linkId = 'openConsole_' + agent.id/]
        <a id='${linkId}'>[@ui.icon type="terminal" textKey="agent.open.terminal.description"/]</a>
        [@linkTerminalButton linkId=linkId agentId=agent.id consolePermissionsPlanKey=consolePermissionsPlanKey/]
    [/#if]
    [#if agent.definition.type.identifier == "elastic"]
        <span id="${agent.id}Elastic">
            [#if permissionManager.canManageElasticBamboo()]
    ...
    [/#if]
    <a href="${req.contextPath}/agent/viewAgent.action?agentId=${agent.id}">${agent.name?html}</a>
    [#if dedicatedLozenge && agent.dedicated] [@ui.agentDedicatedLozenge subtle=true/][/#if]
    [#if missesRequirements ] [@ui.agentDedicatedUnmatchedLozenge subtle=true/][/#if][#t/]
[/#macro]

[#macro linkTerminalButton linkId agentId consolePermissionsPlanKey]
<script>
    var $button = AJS.$("#" + "${linkId}");
    $button.click(
            function(e) {
                e.preventDefault();
                var sessionId = Math.random().toString().substring(2);
                window.open(AJS.contextPath()+"/agent/agentConsole.action?agentId=${agentId}&planKey=${consolePermissionsPlanKey}", "Console #" + sessionId);
            });
</script>
[/#macro]
...
[#macro renderAgentNameAdminLink agent missesRequirements=false]
    [#if agent.definition.type.identifier == "elastic"]
        <span id="${agent.id}Elastic">
            [#if permissionManager.canManageElasticBamboo()]
                <a href="${req.contextPath}/admin/elastic/manageElasticInstances.action">
        ...
        [@dj.tooltip target='${agent.id}Elastic' text="This agent is running in the Amazon Elastic Compute Cloud" /]
    [/#if]
    [#if permissionManager.canManageAgents()]
        <a href="${req.contextPath}/admin/agent/viewAgent.action?agentId=${agent.id}">${agent.name?html}</a>
    [#else]
        ${agent.name?html}
    [/#if]
    [#if agent.dedicated] [@ui.agentDedicatedLozenge subtle=true /][/#if]
    [#if missesRequirements] [@ui.agentDedicatedUnmatchedLozenge subtle=true /][/#if][#t/]
[/#macro]
...
[#macro renderPlanConfigLink plan]
    ...
    [#elseif plan.hasMaster()]
    ${plan.project.name} &rsaquo; ${plan.master.buildName} &rsaquo; [@ui.icon type="devtools-branch" useIconFont=true /] ${plan.buildName}[#t]
    [#else]
...
[@compress singleLine=true]
    [#if req?has_content]
        [#if author.linkedUserName?has_content]
            ${req.contextPath}/users/viewUserSummary.action?currentUserName=${author.linkedUserName?url}
        [#else]
            ${req.contextPath}/authors/viewAuthor.action?authorName=${author.name?url}
        [/#if]
    [#elseif baseUrl?has_content]
        [#if author.linkedUserName?has_content]
            ${baseUrl}/users/viewUserSummary.action?currentUserName=${author.linkedUserName?url}
        [#else]
            ${baseUrl}/authors/viewAuthor.action?authorName=${author.name?url}
        [/#if]
    [/#if]
[/@compress]
[/#macro]
...
[#--
    ...
    @optional useAui - a boolean indicationg whether to use an AUI icon
    @optional extraClasses - any extra css classes to append
    @optional useIconFont - a boolean indicating whether to use the AUI icon font (setting this to true will override 'useAui' parameter)
--]
[#macro icon type text="" textKey="" showTitle=true useAui=false extraClasses="" useIconFont=false]
[#if useAui || useIconFont]${soy.render("aui.icons.icon", {
    "icon": type,
...
[#-- ==================================================================================================== @ui.banner --]
[#--
    Displays a banner in Bamboo. Currently only error banners are supported by AUI.
--]
[#macro banner id="" content="" cssClass=""]
    <div [#if id?has_content]id="${id}"[/#if] class="aui-banner aui-banner-error [#if cssClass?has_content]${cssClass}[/#if]">
        [#if content?has_content]${content}[/#if]
        [#nested /]
    </div>
[/#macro]
...
[#--
    ...
    @requires id - a unique id for the drop down.
--]
[#macro groupedMenu triggerText id triggerId="" cssClass="" icon="" useIconFont=false disabled=false triggerBeforeDropdown=true subtle=false compact=false iconOnly=false arrowless=false]
    [#local trigger]
        <button class="aui-button aui-dropdown2-trigger[#if arrowless] aui-dropdown2-trigger-arrowless[/#if][#if subtle] aui-button-subtle[/#if][#if compact] aui-button-compact[/#if]" aria-owns="${id}" aria-haspopup="true"[#if disabled] aria-disabled="true"[/#if][#if triggerId?has_content] id="${triggerId}"[/#if]>[#rt]
            [#if icon?has_content]
...
[#macro renderWebPanels location params={}]
    [#list ctx.getWebPanels(location, params) as webpanel]
    ${webpanel}
    [/#list]
[/#macro]
...
[#-- =========================================================================================== @ui.copyToClipboard --]
[#-- A flash widget to copy to clipboard.  Parameters text, , copyMessage, copyCompleteMessage         --]
[#-- For example: text=toCopy&copyMessage=copy!&copyCompleteMessage=done.                              --]
[#macro copyToClipboard text="" copyMessage="Click to copy" copyCompleteMessage="Copied!"]
    [#if text?has_content]
        ${soy.render("bamboo.web.resources.common:widget-copy-clipboard", "bamboo.widget.copyclipboard.element", {
        "text": text,
        "copyMessage": copyMessage,
        "copyCompleteMessage": copyCompleteMessage
        })
    }[#t]
    [/#if]
[/#macro]
...
[#macro emptyStateHelp textKey links="" extraClasses=""]
    <div class="aui-help aui-help-empty-state ${extraClasses}">
        <div class="aui-help-content">
            <p>[@ww.text name=textKey /]</p>
