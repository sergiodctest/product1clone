...
[#assign headline = i18n.getText("storage.capping.warning.email.hard.limit.header", [applicationHost]) /]
...
[@atlassianEmailTemplate baseUrl=baseUrl headline=headline]
    <p>${i18n.getText("storage.capping.warning.email.hard.limit.body.part1", [storageLimits.hardLimit])}</p>
    [#if atlassianAgentsEnabled=true]
        <p><strong>${i18n.getText("storage.capping.warning.email.hard.limit.body.part2.aa")}</strong></p>
    [#else]
        <p><strong>${i18n.getText("storage.capping.warning.email.hard.limit.body.part2")}</strong></p>
    [/#if]
    [#if atlassianAgentsEnabled != true]
    ...
    [/#if]
    <p>${i18n.getText("storage.capping.warning.email.hard.limit.body.part3", [policyLink])}</p>
    <p>${i18n.getText("storage.capping.warning.email.hard.limit.body.part4")}</p>
