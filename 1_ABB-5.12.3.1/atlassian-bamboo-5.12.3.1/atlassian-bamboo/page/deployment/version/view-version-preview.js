define('page/deployment/create-version-preview', [
    'jquery',
    'underscore',
    'backbone',
    ...
    var createVersionPreview = (function() {
        var defaults = {
            ...
            selectors: {
                ...
                defaultDeploymentProjectBranchKey: null,
        ...
        updateKeys = function() {
            var defaultDeploymentProjectBranchKey = $(options.selectors.defaultDeploymentProjectBranchKey).val();
            planKey = $branchPicker.auiSelect2('val');
            if (planKey.trim().length == 0) {
                planKey = $(options.selectors.planKeySelector).val();
            }
            deploymentFromMainBranch = (planKey == defaultDeploymentProjectBranchKey);
        },
        ...
        renderVersionNumbers = function(data) {
            $(options.selectors.pickVersionSelector).empty().append(options.versionNumberTemplate({
                ...
                versionNameError: data.errorMessage
            }));
        },
        ...
        return {
            init: function(opts) {
                ...
                events.EventBus.on('branch:change', function() {
                    onPlanBranchChange();
                });
