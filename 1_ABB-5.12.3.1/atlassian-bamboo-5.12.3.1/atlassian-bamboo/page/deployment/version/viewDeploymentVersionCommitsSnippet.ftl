...
<div id="changesSummary" class="changesSummary">
    ...
    [@defaultCommitDisplay changesets repositoryChangeset repositoryData/]
[/#list]
</div>
[#else]
...
[#macro defaultCommitDisplay changesets repositoryChangeset repositoryData]
    [#if repositoryChangeset.commits?has_content]
        [#if action.isWebRepositoryViewerDefined(repositoryData)]
            [#assign commitToUrls = (repositoryData.webRepositoryViewer.getWebRepositoryUrlForDeploymentVersionCommits(repositoryChangeset.commits, repositoryChangeset.repositoryData))! /]
        [/#if]
    <table class="aui code-changes">
        ...
        <tbody>
            ...
            [@displayCommitTableRow commit=commit deploymentVersion=(deploymentVersion!) commitsToUrls=commitToUrls!    /]
        [/#list]
        </tbody>
    </table>
    [/#if]
...
[#macro displayCommitTableRow commit deploymentVersion commitsToUrls]
<tr>
    <td class="avatar">
    ...
    <td class="revision">
        ...
        [#if revision?has_content]
            ...
            [#if commitUrl?has_content]
                <a href="${commitUrl}" class="revision-id" title="[@ww.text name="webRepositoryViewer.viewChangeset" /]">${revision}</a> [@ui.copyToClipboard text=revision /]
            [#else]
