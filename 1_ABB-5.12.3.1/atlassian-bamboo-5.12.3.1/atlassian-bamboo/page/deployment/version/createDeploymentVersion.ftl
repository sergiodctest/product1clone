...
[@ww.form id='createDeploymentVersion' action='doCreateDeploymentVersion' namespace='/deploy' cancelUri='${returnUrl!"/deploy/viewDeploymentProjectEnvironments.action?id=${deploymentProject.id}"}']
    ...
    [@ww.hidden id='defaultDeploymentProjectBranchKey' name='defaultDeploymentProjectBranchKey' /]
    ...
    <div id="${planBranchPickerContainerId}" [#if !masterPlanBranches?has_content || !action.canDeployFromCustomBranch()]class="hidden"[/#if]>
        [@s.textfield
    ...
    [@ui.bambooSection cssClass="select-build-result"]
        ...
        ${soy.render("bamboo.web.resources.common:feature-build-result", "bamboo.feature.build.result.resultSelectionList", {
...
<script type="text/javascript">
    require(['jquery', 'page/deployment/create-version-preview'], function($, createVersionPreview) {
        $(function() {
            createVersionPreview.init({
                deploymentProjectId: ${deploymentProject.id},
                ...
                selectors: {
                    ...
                    defaultDeploymentProjectBranchKey: "#defaultDeploymentProjectBranchKey",
